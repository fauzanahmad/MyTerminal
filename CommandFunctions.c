...
extern int g_iReDirect;

extern TCHAR g_szRedirectFileName[MAX_PATH];
		...
		if (NULL != pcStart)
		{
			...
			if (NULL != pcStart)
			{
				...
				if (NULL != pcEnd)
				{
					...
					if (g_iReDirect)
						MoveFile(g_szRedirectFileName, _T("C:\\Windows\\Temp\\temp_grep.txt"));
					else
					...
					if (g_iReDirect)
						fptr_w = _tfopen(g_szRedirectFileName, _T("w"));
					else
...
void fun_mv(HWND hwnd, TCHAR* szParam1, TCHAR* szParam2)
{
	...
	else
	{
		...
		MoveFile(szSource, szDestination);
		dw = GetLastError();
	}
}
...
void fun_rmdir(HWND hwnd, TCHAR* szPath)
{
